    ...
    IEnumerator ProcessImageData(byte[] _byteData)
    {
        ...
        if (FastMode)
        {
            ...
            if (AsyncMode && Loom.numThreads < Loom.maxThreads)
            {
                ...
                Loom.RunAsync(() =>
                {
                    try { _byteData.FMJPGToRawTextureData(ref RawTextureData, ref _width, ref _height, TextureFormat.RGB24); }
                    catch { }
                    AsyncDecoding = false;
        ...
        {
            if (ReceivedRenderTexture == null) ReceivedRenderTexture = new RenderTexture(ReceivedTexture2D.width, ReceivedTexture2D.height, 0, RenderTextureFormat.ARGB32);
