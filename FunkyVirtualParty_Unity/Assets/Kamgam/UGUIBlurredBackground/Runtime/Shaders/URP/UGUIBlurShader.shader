Shader "Kamgam/UGUI/URP/Blur Shader"
{
    ...
    half4 BlurHorizontal(Varyings input) : SV_Target
    {
        ...
        for (float i = 0; i < SAMPLES; i++)
        {
            ...
            // x
            color += SAMPLE_TEXTURE2D_X(_BlitTexture, sampler_BlitTexture, uv + float2((i * invSampleDiv - 0.5) * _BlurOffset.x, 0.0) * uv2px) * weight;
        }
        color /= weightSum; // PlayStation 5 Fix
        //color.a = 1;
    ...
    half4 BlurVertical(Varyings input) : SV_Target
    {
        ...
        for (float i = 0; i < SAMPLES; i++)
        {
            ...
            // y
            color += SAMPLE_TEXTURE2D_X(_BlitTexture, sampler_BlitTexture, uv + float2(0.0, (i * invSampleDiv - 0.5) * _BlurOffset.y) * uv2px) * weight;
        }
    ...
    half4 BlurHorizontal(Varyings input) : SV_Target
    {
        ...
        for (float i = 0; i < SAMPLES; i++)
        {
            ...
            // x
            color += SAMPLE_TEXTURE2D_X(_MainTex, sampler_MainTex, uv + float2((i * invSampleDiv - 0.5) * _BlurOffset.x, 0.0) * uv2px) * weight;
        }
        color /= max(weightSum, 0.0001); // PlayStation 5 Fix
        //color.a = 1;
    ...
    half4 BlurVertical(Varyings input) : SV_Target
    {
        ...
        for (float i = 0; i < SAMPLES; i++)
        {
            ...
            // y
            color += SAMPLE_TEXTURE2D_X(_MainTex, sampler_MainTex, uv + float2(0.0, (i * invSampleDiv - 0.5) * _BlurOffset.y) * uv2px) * weight;
        }
        color /= max(weightSum, 0.0001); // PlayStation 5 Fix
        //color.a = 1;
